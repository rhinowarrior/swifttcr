...
def superpose_and_change_chain_IDs(reference, target, output):
    """ Superpose the target structure to the reference structure and change the chain IDs based on the alignment. The chain IDs of the target structure will be updated to match the chain IDs of the reference structure. The superposed target structure will be saved to the output file.
    ...
    if alignment:
        ...
        # Apply the chain mapping to the target structure
        for target_chain, ref_chain in chain_mapping.items():
            pymol_cmd.alter(f"target and chain {target_chain}", f"chain='{ref_chain}'")
        
        pymol_cmd.save(output, "target")
