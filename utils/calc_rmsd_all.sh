...
python3 tools/PDB-matching-main/calc_all_models/map_all_pdbs.py "$input_directory" "$reference_structures_directory" "$output_directory_map_pdbs" "$number_of_cores"
...
if [ $? -eq 0 ]; then
    ...
    python3 tools/PDB-matching-main/calc_all_models/calculate_rmsd_all.py "$output_directory_map_pdbs" "$output_file_name" "$number_of_cores"
