...
python3 tools/PDB-matching-main/calc_with_cluster_files/map_pdbs.py "$input_directory" "$reference_structures_directory" "$output_directory_map_pdbs" "$name_clustering_file" "$number_of_cores"
...
if [ $? -eq 0 ]; then
    ...
    python3 tools/PDB-matching-main/calc_with_cluster_files/calculate_rmsd.py "$output_directory_map_pdbs" "$input_directory" "$name_clustering_file" "$output_file_name" "$number_of_cores"
