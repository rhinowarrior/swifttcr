...
def immunopdb_main(input_structure, output_file=None, receptor='tr', scheme='imgt'):
    import os
    import sys

    # Define necessary variables
    switchback = False
    scheme_names = ['imgt', 'kabat', 'chothia', 'martin', 'wolfguy', 'aho']
    ...
    if scheme == 'pdb':
        switchback = scheme
        scheme = 'imgt'
    elif scheme in scheme_short_to_long:
        scheme = scheme_short_to_long[scheme]

    if receptor == 'ig':
        sparser = AntibodyPDBParser(QUIET=True, scheme=scheme)
    elif receptor == 'tr':
        if scheme not in ['imgt', 'i', 'aho', 'a']:
            print('Only imgt or aho schemes can be applied to TCRs', file=sys.stderr)
            sys.exit(1)
        sparser = TcrPDBParser(QUIET=True, scheme=scheme)

    name = os.path.splitext(os.path.basename(input_structure))[0]
    
    try:
        structure = sparser.get_structure(name, input_structure)
    except IOError:
        print(f'File {input_structure} could not be opened', file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(f'{input_structure} could not be parsed:', e, file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        for chain in structure.get_chains():
            print('REMARK   6 '+'\nREMARK   6 '.join(compile_remarks(chain, receptor.upper())), file=of)
        ...
        print(f'Could not write to file {output_file}', file=sys.stderr)
        sys.exit(1)
    except Exception as e:
